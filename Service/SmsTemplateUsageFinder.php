...
/**
 * Finds the campaigns whose "Send via n8n (SMS)" steps point at an
 * SmsTemplate: listed on the template's edit page, and used to refuse
 * deleting a template that is still in use.
 *
 * The template id lives inside the campaign event's serialized
 * 'properties' column (key 'smsTemplate', see Form/Type/
 * SmsDispatchActionType.php), which can't be matched reliably in SQL, so
 * every non-deleted SMS step is loaded and matched in PHP — a handful of
 * rows in practice.
 */
class SmsTemplateUsageFinder
{
    ...
    public function findUsages(int $templateId): array
    {
        $rows = $this->em->getConnection()->createQueryBuilder()
            ...
            ->orderBy('c.name')
            ->addOrderBy('e.id')
            ->setParameter('type', 'n8ndispatch.sms.send')
            ->executeQuery()
            ->fetchAllAssociative();
    ...
    public static function filterRows(array $rows, int $templateId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if (!is_array($properties) || (int) ($properties['smsTemplate'] ?? 0) !== $templateId) {
                continue;
            }
